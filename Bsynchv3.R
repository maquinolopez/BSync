...
BSynch <- function(Input,Target,folder = '~/Documents/BSynch/',
  ...
  if (uq){
     ...
     obj <- function(param){ 
       ...
       obj <- function(param){ 
         ...
         p_sd <- dgamma(my_sd,.2,10,log = T) # this gives almost 95% of prob bertween 0 and .1
       ...
       obj <- function(param){
         my_sd = tail(param,1)
         param =  param[-length(param)]
         -( logprior(param) + loglikelihood(param,my_sd = my_sd) )
       }
     }
  ...
  sampler <-  function(rerun=FALSE){
    ...
    if (double_target){
      return(c(tao0, w, ms,runif(1),runif(1,0,.1)))
    }else{
      if (uq){
        return(c(tao0, w, ms))  
      }else{
        return(c(tao0, w, ms,runif(1,0,.1)))  
      }
    }
  }
  ...
  ## plot initial value tau0  changed to sd
  {
    if (uq){
      d_tao0 <- c[,1]
