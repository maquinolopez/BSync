...
# Plot the density
density_plot <- function(tar_ages, tar_mt,xlabel,ylabel = "proxy units",add = FALSE,axis=TRUE,flip=FALSE,ylim=TRUE){
    ...
    if(axis){
      if (flip){
        if(ylim[1]==TRUE){
          ...
          plot(colMeans(tar_mt),tar_ages , type='l', xlab=ylabel, ylab=xlabel, ylim=ylim,col=rgb(0,0,0,1))  
        }
      }else{
        if(ylim[1]==TRUE){
          ...
          plot(tar_ages,colMeans(tar_mt) , type='l', xlab=xlabel, ylab=ylabel, xlim=c( tar_ages[1],tail(tar_ages,1)),ylim=ylim,col=rgb(0,0,0,1))  
        }
      }
      ...
      if (flip){
        if(ylim[1]==TRUE){
          ...
          plot(colMeans(tar_mt),tar_ages , type='l', ylab=xlabel, xlab=ylabel, ylim=ylim,col=rgb(0,0,0,1),xaxt = 'n')
        }
      }else{
        if(ylim[1]==TRUE){
...
BSynch <- function(Input,Target,folder = '~/Documents/BSynch/',
                   ...
                   n_sections = 100, 
                   sd_input = .01 ,sd_target = .01,
                   tao_mean=TRUE , tao_sd=TRUE ,
                   gw_z=.1,depth_cm = TRUE, age_kyr=TRUE,T_age_kyr =FALSE,
                   ta = 3,tb = 4,
                   uq =FALSE,
                   double_target = FALSE,
                   depth_to_age = TRUE,
  ...
  if (!double_target){
    ...
    tar <- data.frame(X=tar$Age,ProxyValue1 = tar$target1,ProxyValue2 = tar$target2, ProxyValue = c(.47*tar$target2 + .53*tar$target1) )
  ...
  if (normalizar){
    ...
    if(uq){
       tar_mt <- ((max(inp[,2])-min(inp[,2]))/2)*tar_mt
    }
  }
  ...
  my_sd = .01 #sqrt(sd_input^2 +sd_target^2)
  ...
  # loglikelihood
  loglikelihood <- function(params,tar_prox=tar$ProxyValue){
    # print(tar_prox)
    ...
    new_target <- approx(tar$X,tar_prox,t_times)$y
    # Calculate the likelihood
    likelihood <- tdistroC(X = inp$ProxyValue, Mu = new_target, sigma = my_sd, a = ta, b = tb)
    # removed for the C implementation
  ...
  # likelihood in case of UQ
   if (uq){
     ...
     loglikelihood_uq <- function(params){
       ...
       # This ajust the values such that significant offsets do not affect the aligment 
       # adjusted_values <- adjustProxyValues(tar$X, tar$ProxyValue, t_times, window_labels, mean_values, inp$ProxyValue)
       # ll <- loglikelihood_uqCpp(params, adjusted_values, bandwidths, tar_mt, sd_convertor ,loca)
      ...
      # loglikelihood_uq <- function(params){
      # ll <- loglikelihood_uqCfull(params,
      #                         inp$X,inp$ProxyValue,
      #                         bandwidths,
      #                         tar_mt,
      #                         sd_convertor,b_length,breaks,
      #                         tar_ages )
      # return (ll)
      # }
  ...
  if (uq){
    ...
    if (double_target){
      obj <- function(param){ 
        ProxyValue <- range_T( param[length(param)] * tar$ProxyValue1 + (1 - param[length(param)]) * tar$ProxyValue2 )
        param1 <- param[-length(param)]
        return( -(logprior(param1) + loglikelihood(param1,ProxyValue))  )
      }
    }else{
  ...
  sampler <-  function(rerun=FALSE){
    ...
    if(rerun){
      if (depth_to_age){
        ...
        # ms <- rgamma(1, scale = 1/1000 , shape = 1000 )
        tmp_mean <- floor( (tail(tar$X,1)- tar$X[1])/(tail(inp$X,1)-inp$X[1]) )
        ms <- rgamma(1, scale = tmp_mean / shape_acc, shape = shape_acc )
        for (i in 1:(n_slopes-1)){
          # alpha =rgamma(1, scale = 1/ 1000, shape = 1000 )
          alpha = rgamma(1, scale = tmp_mean/ shape_acc, shape = shape_acc )
          ms <- c(  w * ms[1] + (1-w) * alpha, ms)
        }
    }
  ...
  if (run){
    ...
    if (!double_target){
      ...
      c <- output$output[-1,-length(output$output[1,])]
      w_tar <- output$output[-1,length(output$output[1,])]
      output$w_tar <- w_tar
  ...
  # plot energy
  {
    ...
    #par(mgp=c(1.1, 0.5, 0))
    plot(energy,xlab='iterations',ylab='log-likelihood',type = 'l',
         col="grey40",yaxt="n",axes=F)
  ...
  ## plot initial value tau0 
  {
    ...
    curve(dnorm(x, mean = tao_mean, sd = tao_sd), col='lightgray',
          xlab=expression(tau[0]), ylab="Density", main="", yaxt="n",
          from=mintao-10, to=max(d_tao0$x) + 10, ylim=y_range,axes=F)
  ...
  ### Plot the alignment 
  {
    ...
    if (uq){
      density_plot(tar_ages ,tar_mt, xlabel='',flip = F , ylim=y_lim)
    }else{
      if(double_target){
  ...
  if(double_target){
    ...
    text(x = -.005, y = 0, labels = ori_col_names[2], pos = 4,srt = 90, adj = .5,cex=.8) # pos = 1 for below
    text(x = .91, y = 0, labels = ori_col_names[3], pos = 4,srt = 90, adj = .5,cex=.8 ) # pos = 1 for below
